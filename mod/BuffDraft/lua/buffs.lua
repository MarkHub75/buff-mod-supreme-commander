...
-- Every catalog entry has a matching SIM BuffSpec in effects.lua.
...
BuffCatalog = {
    ...
    { id = "reclaim_bonus_1", title = "Reclaim Bonus I", description = "Engineers gain " .. percentChange(n('RECLAIM_RATE_MULT', 2)) .. " reclaim speed and " .. percentChange(n('RECLAIM_YIELD_MULT', 2)) .. " mass/energy yield.", effect = percentChange(n('RECLAIM_RATE_MULT', 2)) .. " reclaim speed while engineers reclaim and " .. percentChange(n('RECLAIM_YIELD_MULT', 2)) .. " total reclaim yield.", target = "reclaim" },

    -- tier-II upgrades: available only after their prerequisite is picked and
    -- weighted more heavily than other buffs inside the same rarity bucket
    { id = "shield_health_2", title = "Shield Health II", description = "Shield structures gain another " .. percentChange(n('SHIELD_HEALTH_2_HEALTH_MULT', 1.5)) .. " maximum shield HP and +" .. string.format("%g", (n('SHIELD_HEALTH_2_AREA_MULT', 1.4) - 1) * 100) .. "% shield area.", effect = percentChange(n('SHIELD_HEALTH_2_HEALTH_MULT', 1.5)) .. " maximum shield HP and +" .. string.format("%g", (n('SHIELD_HEALTH_2_AREA_MULT', 1.4) - 1) * 100) .. "% shield area, stacking with Shield Health I.", target = "shields", prerequisite = "shield_health_1", draftWeight = n('Tier2DraftWeight', 3) },
    { id = "tactical_range_2", title = "Tactical Range II", description = "Tactical missiles gain " .. string.format("%g", n('TACTICAL_RANGE_2_DAMAGE_RADIUS_MULT', 3)) .. "x explosion radius.", effect = string.format("%g", n('TACTICAL_RANGE_2_DAMAGE_RADIUS_MULT', 3)) .. "x tactical-missile explosion radius, stacking with Tactical Range I.", target = "tactical", prerequisite = "tactical_range_1", draftWeight = n('Tier2DraftWeight', 3) },
    { id = "air_speed_2", title = "Air Speed II", description = "All mobile aircraft weapons reload " .. string.format("%g", n('AIR_SPEED_2_ROF_MULT', 2)) .. "x faster.", effect = string.format("%g", n('AIR_SPEED_2_ROF_MULT', 2)) .. "x rate of fire for aircraft weapons, including bombs, stacking with Air Speed I.", target = "air", prerequisite = "air_speed_1", draftWeight = n('Tier2DraftWeight', 3) },
    { id = "acu_regen_2", title = "ACU Regeneration II", description = "ACUs gain +" .. n('ACU_REGEN_2_HEALTH_ADD', 5000) .. " maximum HP.", effect = "+" .. n('ACU_REGEN_2_HEALTH_ADD', 5000) .. " maximum HP on ACUs, stacking with ACU Regeneration I.", target = "acu", prerequisite = "acu_regen_1", draftWeight = n('Tier2DraftWeight', 3) },
    { id = "artillery_range_2", title = "Artillery Range II", description = "Artillery gains " .. string.format("%g", n('ARTILLERY_RANGE_2_DAMAGE_RADIUS_MULT', 2)) .. "x explosion radius and " .. percentChange(n('ARTILLERY_RANGE_2_ROF_MULT', 1.4)) .. " rate of fire.", effect = string.format("%g", n('ARTILLERY_RANGE_2_DAMAGE_RADIUS_MULT', 2)) .. "x indirect-fire explosion radius and " .. percentChange(n('ARTILLERY_RANGE_2_ROF_MULT', 1.4)) .. " rate of fire, stacking with Artillery Range I.", target = "artillery", prerequisite = "artillery_range_1", draftWeight = n('Tier2DraftWeight', 3) },
    { id = "mobile_shields_2", title = "Mobile Shields II", description = "Mobile shield generators gain " .. percentChange(n('MOBILE_SHIELDS_2_HEALTH_MULT', 3)) .. " maximum shield HP and " .. percentChange(n('MOBILE_SHIELDS_2_RADIUS_MULT', 1.3)) .. " shield radius.", effect = percentChange(n('MOBILE_SHIELDS_2_HEALTH_MULT', 3)) .. " maximum shield HP and " .. percentChange(n('MOBILE_SHIELDS_2_RADIUS_MULT', 1.3)) .. " bubble radius, stacking with Mobile Shields I.", target = "mobile_shields", prerequisite = "mobile_shields_1", draftWeight = n('Tier2DraftWeight', 3) },
    { id = "naval_armor_2", title = "Naval Armor II", description = "Mobile naval units gain another " .. percentChange(n('NAVAL_ARMOR_2_HEALTH_MULT', 3)) .. " maximum HP.", effect = percentChange(n('NAVAL_ARMOR_2_HEALTH_MULT', 3)) .. " maximum HP on mobile naval units, stacking with Naval Armor I.", target = "naval", prerequisite = "naval_armor_1", draftWeight = n('Tier2DraftWeight', 3) },
    { id = "experimentals_health_2", title = "Experimental Health II", description = "Experimental units gain another " .. percentChange(n('EXPERIMENTAL_HEALTH_2_HEALTH_MULT', 2)) .. " maximum HP and +" .. n('EXPERIMENTAL_HEALTH_2_REGEN_ADD', 15) .. " HP/s regeneration.", effect = percentChange(n('EXPERIMENTAL_HEALTH_2_HEALTH_MULT', 2)) .. " maximum HP and +" .. n('EXPERIMENTAL_HEALTH_2_REGEN_ADD', 15) .. " HP/s on experimentals, stacking with Experimental Health I.", target = "experimentals", prerequisite = "experimentals_health_1", draftWeight = n('Tier2DraftWeight', 3) },
    ...
    { id = "overcharged_shields_1", title = "Overcharged Shields", description = "All shielded units gain " .. percentChange(n('OVERCHARGED_SHIELD_MULT', 2.5)) .. " maximum shield HP and shield regeneration.", effect = percentChange(n('OVERCHARGED_SHIELD_MULT', 2.5)) .. " maximum shield HP and regeneration rate on all shielded units, preserving their normal full-regeneration time.", target = "shields" },
    { id = "napalm_rounds_1", title = "Napalm Rounds", description = "Direct-fire land and artillery weapons gain +" .. n('NAPALM_RADIUS_ADD', 1) .. " splash radius and burn for " .. n('NAPALM_DOT_TIME', 3) .. " seconds.", effect = "+" .. n('NAPALM_RADIUS_ADD', 1) .. " damage radius and " .. n('NAPALM_DOT_PULSES', 3) .. " total damage pulses over " .. n('NAPALM_DOT_TIME', 3) .. " seconds.", target = "land" },
    { id = "mobility_doctrine_1", title = "Mobility Doctrine", description = "ACUs, SCUs and engineers gain " .. percentChange(n('MOBILITY_DOCTRINE_SPEED_MULT', 2)) .. " movement speed.", effect = percentChange(n('MOBILITY_DOCTRINE_SPEED_MULT', 2)) .. " movement speed on ACUs, SCUs and engineers.", target = "engineers" },
    { id = "missile_storm_1", title = "Missile Storm", description = "Tactical launchers fire " .. n('MISSILE_STORM_PROJECTILES', 3) .. " missiles per shot with " .. percentChange(n('MISSILE_STORM_ROF_MULT', 2.5)) .. " rate of fire and " .. percentChange(n('MISSILE_STORM_BUILD_MULT', 2.5)) .. " missile build rate.", effect = n('MISSILE_STORM_PROJECTILES', 3) .. " projectiles per launcher shot, " .. percentChange(n('MISSILE_STORM_ROF_MULT', 2.5)) .. " rate of fire and " .. percentChange(n('MISSILE_STORM_BUILD_MULT', 2.5)) .. " missile construction rate.", target = "tactical" },
    ...
    { id = "nano_swarm_1", title = "Nano Swarm", description = "All units gain +" .. n('NANO_REGEN_ADD', 5) .. " HP/s after " .. n('NANO_OUT_OF_COMBAT_DELAY', 10) .. " seconds without taking damage.", effect = "+" .. n('NANO_REGEN_ADD', 5) .. " HP/s out of combat; taking damage disables it for " .. n('NANO_OUT_OF_COMBAT_DELAY', 10) .. " seconds.", target = "all" },
    { id = "experimental_discount_1", title = "Experimental Assembly", description = "Experimentals cost " .. percentChange(n('EXP_DISCOUNT_COST_MULT', 0.5)) .. " resources and their builders gain " .. percentChange(n('EXP_DISCOUNT_BUILD_MULT', 2.5)) .. " build rate.", effect = percentChange(n('EXP_DISCOUNT_COST_MULT', 0.5)) .. " mass/energy cost plus " .. percentChange(n('EXP_DISCOUNT_BUILD_MULT', 2.5)) .. " builder rate while constructing an experimental.", target = "experimentals" },
    ...
    { id = "chain_lightning_weapons_1", title = "Chain Lightning", description = "Beam weapons gain " .. percentChange(n('CHAIN_BEAM_DAMAGE_MULT', 1.5)) .. " damage and arc to up to " .. n('CHAIN_ARC_TARGETS', 2) .. " nearby enemies.", effect = percentChange(n('CHAIN_BEAM_DAMAGE_MULT', 1.5)) .. " beam damage, +" .. n('CHAIN_BEAM_RADIUS_ADD', 1.5) .. " splash radius, then " .. percentChange(n('CHAIN_ARC_DAMAGE_FRACTION', 0.5)) .. " tick damage to up to " .. n('CHAIN_ARC_TARGETS', 2) .. " enemies within " .. n('CHAIN_ARC_RADIUS', 12) .. ".", target = "land" },
    ...
    { id = "salvage_explosion_1", title = "Salvage Explosion", description = "Enemy kills leave " .. percentChange(n('SALVAGE_RECLAIM_MULT', 2)) .. " richer wreckage and have a " .. n('SALVAGE_EXPLOSION_CHANCE', 25) .. "% chance to explode.", effect = percentChange(n('SALVAGE_RECLAIM_MULT', 2)) .. " wreckage mass/energy; " .. n('SALVAGE_EXPLOSION_CHANCE', 25) .. "% chance for " .. n('SALVAGE_EXPLOSION_DAMAGE', 150) .. " area damage in radius " .. n('SALVAGE_EXPLOSION_RADIUS', 3) .. ", no friendly fire.", target = "economy" },
