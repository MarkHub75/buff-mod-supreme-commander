...
## Warning-log triage (source: warnings/Text document.txt, FAF Maui button.lua, M28AI M28Micro.lua/M28Events.lua)
- A popup must not destroy its clicked Maui button synchronously from `Button.OnClick`: stock `Button.OnRelease` calls `self:Play()` immediately after `OnClick` returns. Defer popup destruction by one UI frame to avoid `Error running OnRelease script in <deleted object> ... button.lua(119): Game object has been destroyed`.
- Repeated `Unit:GetCargo only valid for transport units` traces in the inspected log originate in M28AI, not BuffDraft. M28 calls `ConsiderSurfacingAtlantisToReleaseCargo` for `refCategoryMobileAircraftFactory`, which is composed of `EXTERNALFACTORYUNIT` categories; those helper factory units are not necessarily transports even though the Lua method is present. Do not globally override `Unit:GetCargo` in BuffDraft to hide this third-party error.

## Persistent strategic warning markers (source: lua/SimPing.lua, lua/ui/game/nukelaunchping.lua)
- FAF's nuclear target warning uses the SIM mesh `/meshes/game/ping_nuke_marker`; `SimPing.SpawnPingMesh` places it on a bare `Entity`, forces visibility, and pulses its scale in a SIM thread.
- A permanent fog-visible warning can use the same mesh on an entity owned by the target unit, with all four visibility relations set to `Always`. Keeping that entity in the unit's `Trash` removes it safely on death/cancel, while a short SIM tracking thread keeps the marker over mobile experimental artillery.
- Stock lobby restriction categories define the intended sets precisely: T3 artillery is `ARTILLERY * STRUCTURE * TECH3`; T4 artillery is `(ARTILLERY * EXPERIMENTAL * STRATEGIC) - FACTORY`. Yolona Oss is `xsb2401` and is a strategic experimental nuke rather than an artillery-category unit.
...
- **`wep:AddDamageMod(add)` / `wep:AddDamageRadiusMod(add)`** — плоские per-instance добавки к damage table оружия. `Weapon:GetDamageTable()` возвращает кэш конкретного экземпляра оружия, поэтому SIM-хук может безопасно задать ему `DoTTime`/`DoTPulses`. `MuzzleSalvoSize` остаётся blueprint-only, но дополнительный projectile можно создать повторным вызовом оригинального `Weapon.CreateProjectileForWeapon` для того же экземпляра оружия.
- **Per-side economy без blueprint mutation**: `Unit:GetBuildCosts(targetBp)` вызывается на конкретном строителе и возвращает time/energy/mass — стоимость экспериментала можно масштабировать только для армии владельца баффа. `Unit:GetReclaimCosts(target)` и `Prop:GetReclaimCosts(reclaimer)` аналогично получают конкретного реклеймера, поэтому yield можно менять side-safe.
- **Телепорт без enhancement blueprint**: штатный `Unit:OnTeleportUnit` проверяет `TestCommandCaps('RULEUCC_Teleport')`, а стоимость/время берёт из общей `TeleportCostFunction` с fallback. В BuffDraft этот путь оставлен только для соответствующего пакета `commander_apotheosis_1`; `mobility_doctrine_1` команду телепорта не выдаёт.
...
## Buff implementation matrix (audit, catalog = 46 buffs)

Источник: mod/BuffDraft/lua/buffs.lua (каталог/пул драфта), effects.lua (BuffSpecs + NotImplementedReasons).
Сверка автоматическая: 46 id каталога = 46 BuffSpecs, NotImplementedReasons пуст, орфанов нет в обе стороны;
34 статических BuffBlueprint и 4 динамических blueprint пакетов командира зарегистрированы и используются. Пул драфта = весь каталог.
...
| shield_health_2 | implemented | STRUCTURE × SHIELD | да | да | да | ещё x1.5 HP щита; x√1.4 радиус = +40% площади купола |
| tactical_range_2 | implemented | TACTICALMISSILEPLATFORM (WeaponCategory=Missile) | да | да | да | x3 радиус взрыва ракеты |
| air_speed_2 | implemented | AIR × MOBILE (боевые оружия) | да | да | да | x2 RoF, включая авиационные бомбы |
| acu_regen_2 | implemented | COMMAND | да | да | да | +5000 MaxHealth |
| artillery_range_2 | implemented | ARTILLERY (UWRC_IndirectFire) | да | да | да | x2 радиус взрыва + x1.4 RoF |
| mobile_shields_2 | implemented | MOBILE × SHIELD | да | да | да | ещё x3 HP щита + x1.3 радиус купола; personal shield не имеет площади защиты |
| naval_armor_2 | implemented | NAVAL × MOBILE | да | да | да | ещё x3 MaxHealth |
| experimentals_health_2 | implemented | EXPERIMENTAL | да | да | да | ещё x2 MaxHealth +15 HP/s |
...
| overcharged_shields_1 | implemented | ALLUNITS с unit.MyShield | да | да | да | x2.5 HP и regen щита через concrete shield entity; время полного восстановления сохраняется |
| napalm_rounds_1 | implemented | LAND × MOBILE (direct) + ARTILLERY (indirect) | да | да | да | +1 радиус; 3 damage pulses за 3с через per-weapon damage table |
| mobility_doctrine_1 | implemented | ENGINEER + COMMAND + SUBCOMMANDER | да | да | да | x2 скорость; телепорт не выдаётся |
| missile_storm_1 | implemented | TACTICALMISSILEPLATFORM | да | да | да | 3 projectile на выстрел + x2.5 RoF + x2.5 missile build rate; без blueprint mutation |
...
| nano_swarm_1 | implemented | ALLUNITS | да | да | поток 1/армию | +5 regen после 10с без урона; OnDamage снимает эффект немедленно |
| experimental_discount_1 | implemented | билдеры ENGINEER + FACTORY, цели EXPERIMENTAL | да** | да | Stacks=IGNORE + checked remove | x2.5 build rate и x0.5 mass/energy cost через GetBuildCosts конкретного билдера |
...
| chain_lightning_weapons_1 | implemented | все юниты с beam-оружием (bp.BeamLifetime) | да | да | да | x1.5 урон +1.5 splash; каждый beam tick бьёт до 2 вторичных целей в r=12 за 50% урона |
...
| salvage_explosion_1 | implemented | килы любых юнитов армии | да | да | флаг 1/армию | x2 mass/energy конкретного созданного wreckage + 25% взрыв r=3/150 без friendly fire |
| reclaim_bonus_1 | implemented | билдеры ENGINEER (во время реклейма) | да | да | Stacks=IGNORE + checked remove | x2 скорость через условный BuildRate и x2 yield через GetReclaimCosts конкретного реклеймера |
| paragon_singularity_1 | implemented | army economy | immediate | n/a | generation token per army | Stock Paragon adaptive formula through per-tick GiveResource: 20..10000 mass/s and 1000..1000000 energy/s. |
| commander_apotheosis_1 | implemented | COMMAND | package purchase | n/a | SIM-owned package state | Four stackable faction packages, all 46 source enhancement costs/times, safe instance-only equivalents, separate mythic UI. |
| nuclear_apocalypse_1 | implemented | NUKE launchers/projectiles | yes | yes | per-army flag + BuffType | x6 inner/outer NukeAOE damage, x3 both radii, x1.5 launcher BuildRate; no blueprint mutation. |

Итог: 46 implemented, 0 partial, 0 not implemented. Все 46 баффов каталога имеют эффект;
orbital_lance_1 дополнительно помечен как active, потому что запускается игроком и имеет cooldown.
Баффы в каталоге без эффекта: нет.
Эффекты в коде вне каталога: нет.

## Tier-II per-instance APIs (source: lua/shield.lua, lua/sim/weapon.lua, lua/sim/Buff.lua)

- `Weapon:AddDamageRadiusMod(delta)` хранит плоскую прибавку на конкретном weapon instance. Для множителя радиуса BuffDraft хранит combined multiplier отдельно и добавляет только разницу `blueprintRadius * (newCombined - oldCombined)`: так множители корректно стекаются и снимаются, не затирая плоский радиус Napalm Rounds.
- У bubble shield поле `shield.Size` — диаметр. `Shield.CreateShieldMesh` создаёт collision sphere радиусом `Size / 2` и ставит draw scale `Size` на основной mesh и `MeshZ`. Поэтому side-safe изменение площади/радиуса повторяет эти три операции на конкретном shield entity; blueprint не меняется. Personal shield не имеет защищаемой площади — его урон маршрутизируется через owner, поэтому радиус к нему неприменим.
- `BuffCalculate` перемножает все `Mult` одного affect type, поэтому отдельные MaxHealth-баффы II уровня естественно стекаются с I уровнем. `Add` считается до множителей; ACU Regeneration II использует штатный `MaxHealth.Add=5000`.
...
- **mythic (3)**: paragon_singularity_1, commander_apotheosis_1, nuclear_apocalypse_1. Unlocks at choice #10, uses a fixed 5% roll window, at most one slot per choice, then a 10-choice offer cooldown.
...
- **rare (15)**: engineer_build_speed_1, factory_build_speed_1, land_rate_of_fire_1, eco_overclock_1, naval_armor_1, naval_armor_2, experimentals_health_1, experimentals_health_2, overcharged_shields_1, napalm_rounds_1, missile_storm_1, nano_swarm_1, rapid_deployment_1, hunter_protocol_1, black_market_economy_1.
- **common (20)**: всё остальное, включая шесть common-апгрейдов II уровня: shield_health_2, tactical_range_2, air_speed_2, acu_regen_2, artillery_range_2, mobile_shields_2.
...
1. Номер choice стороны = picked + pending + 1. rare/legendary доступны с choice #6, mythic — с choice #10.
2. Один общий **rarity pattern на тик** для обеих сторон (одинаковые редкости слотов, НЕ одинаковые баффы). Фиксированные непересекающиеся окна: mythic 5% (максимум 1), legendary 20% (максимум 1), rare 35%, иначе common. Заблокированное окно не увеличивает шанс другого тира.
3. Кулдауны начинаются после ПОЯВЛЕНИЯ опции: legendary — 3 последующих resolved-choice, mythic — 10. Сам choice с оффером и admin grant кулдаун не уменьшают; у rare кулдауна нет.
...
6. Бафф II уровня попадает в available pool только после уже сделанного пика соответствующего I уровня. После этого его вес равен `Tier2DraftWeight=3` против веса 1 у обычного баффа той же редкости; rarity pattern и шансы редкостей от этого не меняются. Реролл использует тот же алгоритм.
- UI: rare — зелёный `FF80FF80`, legendary — фиолетовый `FFC080FF`; mythic — уникальный `FFFF4FD8`, фон `D0380828`, золотой акцент и шрифт `Arial Bold`, плюс явный текстовый тег. History, активная строка и отдельное окно командира повторяют mythic-оформление.

## Balance-конфиг (mod/BuffDraft/lua/config.lua, регион «balance knobs»)
- Все балансные числа мода — named-поля в config.lua: `DraftIntervalSeconds` (300 = один выбор каждые 5 минут), `OptionsPerTick`, `Tier2DraftWeight`, все множители баффов (имена совпадают с локалами effects.lua: ENGINEER_BUILD_RATE_MULT и т.д.), интервалы спавна, параметры salvage и orbital lance.
...
- **Кнопка Admin owner-only, но visibility надо пересчитывать после создания панели**: `GetArmiesTable()`/nickname может быть ещё не готов в первый UI tick. Создавать кнопку по `DebugAdmin`, держать скрытой до совпадения с одним из `AdminOwnerNicknames`, пересчитывать layout на sync events; `admin.Open()` и SIM callbacks всё равно проверяют owner-gate.
...
- **Owner-gate админки** (`AdminOwnerNicknames` в config.lua, единая проверка `lua/admin_access.lua`): UI — `GetArmiesTable()` возвращает `{ armiesTable = { [i] = { nickname, civilian, ... } }, focusArmy = n }` (score.lua:409, поле nickname:299); ник локального игрока = `armiesTable[focusArmy].nickname`. SIM — `ArmyBrains[senderArmy].Nickname` (заполняется в OnCreateArmyBrain), senderArmy строго из GetCurrentCommandSourceArmy → UI-обход невозможен. Пустой список отключает ник-проверку (остаётся DebugAdmin). Сейчас разрешены ровно `Mnogoruchka` и `Mnogonozhka`. Ограничение: ник наблюдателя/реплея не проверяется — но такие отправители отсекаются раньше по nil senderArmy.
...
- **Shield-баффы должны трекать конкретный `unit.MyShield.EntityId`, а не только unit marker**: shield entity создаётся/пересоздаётся через `Unit.CreateShield` (OnStopBeingBuilt, upgrades/enhancements). Если custom marker поставить до реального `SetMaxHealth` или оставить marker от старого shield entity, новый щит не получит активный бафф. Рабочий паттерн BuffDraft: `requireShield + tracksShieldEntity`, hook `CreateShield`/`OnShieldEnabled`, helper возвращает success только после изменения shield max.
- **ACU regen использует штатный Buff.lua и совместим с enhancement-регеном**: `BuffDraftAcuRegen1` имеет `EntityCategory = 'COMMAND'`, постоянный flat `Regen.Add`; `BuffEffects.Regen` пересчитывает итог через все активные Regen affects. Для полевого теста `ApplyPickedBuff` пишет per-part audit (`matched/applied/already/unfinished/failed`) и отдельный `commander audit expectedRegenBonus/appliedCommanders`, чтобы отличить отсутствие ACU в armies от отказа `Buff.ApplyBuff`.
...
## Передача юнитов между армиями (AI control tool; source: lua/SimUtils.lua, SimCallbacks.lua, engine/User.lua, lua/ui/game/chat/commands/builtin/GiftUnits.lua)
...
- Vanilla `/gift-units` тоже использует `GetRolloverInfo()` как fallback-цель под курсором, если игрок
  не указал target явно; это подтверждает rollover как штатный UI-паттерн для cursor-target команд.
- **`GetRolloverInfo()` может очиститься на том же клике, который завершает command mode `ping`**: брать rollover только внутри end behavior ненадёжно. Инструмент Take AI теперь раз в UI-frame запоминает последнюю валидную `{entityId, blueprintId}` во время своего режима и использует её только как fallback. SIM сверяет cached entity с координатой клика и размером blueprint, поэтому старое наведение не может забрать случайный юнит после клика по земле. UI `blueprintId='unknown'` не используется как stale-id guard.
- Результат Take AI (`transferred` или точная причина отказа: nickname/alliance/ACU/attached/transfer filter и т.д.) возвращается из SIM через `Sync.BuffDraft` только армии-отправителю; это убирает прежний молчаливый отказ.
- **`GetSelectedUnits()` (UI) возвращает только СВОИ юниты** — движок не даёт выделять юнитов
  ...
  альтернатива для «забрать у AI»: клик по юниту/зданию через command mode 'ping', UI отправляет
  `GetRolloverInfo().entityId` + `blueprintId`, SIM делает `GetEntityById(id)`, сверяет blueprint
  против stale/recycled id и передаёт ровно эту entity через `TransferUnitsOwnership({ target }, senderArmy)`.
  Допустимый target в BuffDraft: `ALLUNITS - INSIGNIFICANTUNIT`, то есть land/naval/air/structures;
  ACU и experimentals отдельно gated конфигом.
  Радиусная выборка (`GetUnitsAroundPoint`) для этого UX плоха: забирает соседей и не подходит для
  зданий, если категория ограничена `MOBILE`.
...
- `tech-ai.lua:498`, `platoon.lua:1528`, `platoon.lua:3337`, and `*AI WARNING: ... Invalid location` are core AI manager/platoon failures. BuffDraft does not mutate `BuilderManagers`, but `EnableAIDirector=true`, `AIDirectorOrdersEnabled=true`, and `EnableAIControl=true` can indirectly stress AI state. The old `AIControl` radius mode used `LAND * MOBILE`, so it could transfer nearby allied AI engineers; the exact-target mode avoids the radius spillover and can target structures.
...
## Mythic buff implementation findings

- Stock Paragon (`XAB1401_script.lua`) recalculates production every sim tick: base 20 mass/s and 1000 energy/s plus the gap between requested and non-Paragon income, capped by blueprint `MaxMass=10000` and `MaxEnergy=1000000`. An entity-free equivalent can use the same calculation and `brain:GiveResource` once per tick; unlike `SetProductionPerSecond*`, those grants do not need the previous Paragon output subtracted from reported economy income.
- Strategic nuke inner/outer damage is not taken from the ordinary projectile damage table. `/lua/sim/weapon.lua:CreateProjectileForWeapon` constructs two per-projectile `NukeAOE` objects with mutable `Damage` and `Radius` fields. Scaling those objects after the stock constructor is the side-safe solution; changing weapon blueprints would affect every army.
- Stock ACU enhancements are defined on four different unit blueprints and depend on faction scripts, bones, weapons and mutually exclusive slots. Cross-faction installation through the stock enhancement UI is unsafe. BuffDraft therefore uses four SIM-owned packages whose costs/times are summed from all 46 non-remove enhancement blueprints and whose effects are stackable per-instance equivalents. UI only requests a package id; SIM derives the sender army and validates ownership, commander state and economy event.
