...
## Buff implementation matrix (audit, catalog = 38 buffs)

Источник: mod/BuffDraft/lua/buffs.lua (каталог/пул драфта), effects.lua (BuffSpecs + NotImplementedReasons).
Сверка автоматическая: 38 id каталога = 38 BuffSpecs, NotImplementedReasons пуст, орфанов нет в обе стороны;
30 статических BuffBlueprint и 4 динамических blueprint пакетов командира зарегистрированы и используются. Пул драфта = весь каталог.
...
| overcharged_shields_1 | partial | ALLUNITS с unit.MyShield | да | да | да | x2.5 щит; перезарядка не сделана — shield spec фиксирован при создании |
...
Итог: 29 implemented, 9 partial, 0 not implemented (orbital_lance_1 → implemented после
добавления ручного наведения). Все 38 баффов каталога имеют эффект;
